...
@implementation ZXWebImageDBUtil

static sqlite3 *_db = nil;

#define TBL_ZXWEBIMAGE_DB @"TBL_ZXWEBIMAGE_DB"
...
+ (sqlite3 *)open {
    ...
    // 生成数据库文件在沙盒中的路径
    NSString *sqlPath = [docPath stringByAppendingPathComponent:@"zxwebimage.sqlite"];
    ...
    if ([fileManager fileExistsAtPath:sqlPath] == NO) {
        // 获取数据库文件在包中的路径
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"zxwebimage" ofType:@"sqlite"];
...
+ (void)initialize {
    sqlite3 *db = [ZXWebImageDBUtil open];
    
    NSString *sql = [NSString stringWithFormat:@"CREATE TABLE if not exists '%@' ('id' INTEGER PRIMARY KEY AUTOINCREMENT  NOT NULL , 'url' VARCHAR(%zd), 'cachePath' VARCHAR(%zd))", TBL_ZXWEBIMAGE_DB, (long)STRING_MAXLEN, (long)STRING_MAXLEN];
    // 执行sql语句
    int result = sqlite3_exec(db, sql.UTF8String, nil, nil, nil);
    
    if (result == SQLITE_OK) {
        NSLog(@"建表ZXWEBIMAGE成功");
    } else {
        NSLog(@"建表ZXWEBIMAGE失败");
    }
    
    [ZXWebImageDBUtil close];
}
...
+ (void)insertRecordWithURL:(NSString *)url cacheName:(NSString *)cacheName {
    ...
    sqlite3 * db = [ZXWebImageDBUtil open];
    NSString * sql = [NSString stringWithFormat:@"insert into %@ (url, cachePath) values(?, ?) ", TBL_ZXWEBIMAGE_DB];
    // 创建一个语句对象
...
+ (NSString *)queryRecordWithURL:(NSString *)url {
    ...
    // 创建一个语句对象
    sqlite3_stmt *stmt = nil;
    NSString * sql = [NSString stringWithFormat:@"select * from %@ where url = (?)", TBL_ZXWEBIMAGE_DB];
    // 生成语句对象
    int result = sqlite3_prepare_v2(db, sql.UTF8String, -1, &stmt, nil);
...
+ (void)deleteRecordWithURL:(NSString *)url {
    sqlite3 * db = [ZXWebImageDBUtil open];
    NSString * sql = [NSString stringWithFormat:@"delete from %@ where url in (?)", TBL_ZXWEBIMAGE_DB];
    sqlite3_stmt *stmt = nil;
